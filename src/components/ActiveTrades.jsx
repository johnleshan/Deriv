    ...
    <div className="active-trades glass animate-fade-in" style={{ padding: '24px', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '20px', gridColumn: 'span 2' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: '600', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Briefcase size={20} /> Recent Positions
        </h3>
        <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{trades.length} Recent Trades</span>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', minHeight: '100px' }}>
        {trades.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', fontSize: '0.9rem', paddingTop: '20px' }}>
          ...
          trades.map(trade => (
            <div key={trade.id} className="trade-item glass" style={{ padding: '12px 20px', borderRadius: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', alignItems: 'center' }}>
              ...
              <span style={{ color: 'var(--text-secondary)' }}>{trade.symbol.replace('_', ' ')}</span>
              <span style={{ fontWeight: '600' }}>${trade.stake}</span>
              <span style={{ color: trade.profit >= 0 ? 'var(--success)' : 'var(--danger)', fontWeight: '700' }}>
