...
const AccountCreation = ({ onCancel }) => {
  const { verifyEmail, createVirtualAccount } = useDeriv();
      ...
      <div className="glass" style={{ padding: '40px', borderRadius: '24px' }}>
        ...
        {step === 1 && (
          <form onSubmit={handleSendEmail} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <button className="btn-primary" type="submit" disabled={loading} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
